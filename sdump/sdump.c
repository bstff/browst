...
int sixel_data_write_callback(char *data, int size, void *priv)
{
	struct cb_data *d = (struct cb_data*)priv;
	if(d->data == NULL){
		d->data = (char*)malloc(size);
		d->size = 0;
	}else{
		char *buff = (char*)malloc(d->size + size);
		memcpy(buff, d->data, d->size);
		char *tmp = d->data;
		d->data = buff;
		free(tmp);
	}
	...
	d->size += size;
	// fwrite(d->data, d->size, 1, stdout);
	return size;
}

bool load_png_from_mem(unsigned char* mem, const size_t size, struct image *img){
	if(!mem || size == 0){
		return false;
	}
...
uint8_t *crop_image_single(struct image *img, uint8_t *data,
...
//export function
void *image_from_file(char *file){
...
void *image_from_data(unsigned char *data, int size){

	struct image *img = (struct image*)malloc(sizeof(struct image));
	/* init */
	init_image(img);

	if (load_png_from_mem(data, size, img) == false) {
		logging(FATAL, "couldn't load image\n");
		return NULL;
	}
...
void image_rotate(void *handle, int angle){
	struct image *img = (struct image*)handle;
	rotate_image(img, angle, true);
}

void image_resize(void *handle, int width, int height){
	struct image *img = (struct image*)handle;

	resize_image(img, width, height, true);
}

void image_crop(void *handle, int x, int y, int w, int h){

	struct image *img = (struct image*)handle;

	uint8_t *cropped_data = crop_image_single(img, img->data[img->current_frame],
		x, y, w, h);

	img->data[img->current_frame] = cropped_data;
}

void *sixel_output2(void *handle, int *output_size){
	struct image *img = (struct image*)handle;
	...
	struct cb_data data;
	data.data = NULL;
	if ((sixel_context = sixel_output_create(sixel_data_write_callback, &data)) == NULL) {
		logging(ERROR, "couldn't create sixel context\n");
		goto error_occured;
	}
	sixel_output_set_8bit_availability(sixel_context, CSIZE_7BIT);
	...
	for (int i = 0; i < get_frame_count(img); i++) {
		// printf("\0338"); /* restore cursor position */
		sixel_encode(get_current_frame(img), get_image_width(img), 
			get_image_height(img), get_image_channel(img), sixel_dither, sixel_context);
	...
	*output_size = data.size;
	// fwrite(data.data, *size, 1, stdout);
