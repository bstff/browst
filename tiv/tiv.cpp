...
void realloc_color_data(struct color_data *data){
  if(data->pos + 64 > data->size){

    data->size = data->pos + 512;
    char *buf = (char*)malloc(data->size);

    char *fbuf = data->buff;
    memcpy(buf, fbuf, data->pos);
    data->buff = buf;
    free(fbuf);
  }
}

void emit_color(struct color_data *data, int flags, int r, int g, int b) {
  realloc_color_data(data);
  ...
  if ((flags & FLAG_MODE_256) == 0) {
    ...
    data->pos += sprintf(data->buff+data->pos, "%s%d;%d;%dm",
      bg ? "\x1b[48;2;" : "\x1b[38;2;",
      r,g,b);
    return;
  }
  ...
  data->pos += sprintf(data->buff+data->pos, "%s%dm", 
    bg ? "\x1B[48;5;" : "\u001B[38;5;",color_index);
}


void emitCodepoint(struct color_data *data, int codepoint) {
  realloc_color_data(data);

  if (codepoint < 128) {
    // std::cout << (char) codepoint;
    data->buff[data->pos++] = (char) codepoint;
  } else if (codepoint < 0x7ff) {
    // std::cout << (char) (0xc0 | (codepoint >> 6));
    // std::cout << (char) (0x80 | (codepoint & 0x3f));

    data->buff[data->pos++] = (char) (0xc0 | (codepoint >> 6));
    data->buff[data->pos++] = (char) (0x80 | (codepoint & 0x3f));
    ...
    data->buff[data->pos++] = (char) (0xe0 | (codepoint >> 12));
    data->buff[data->pos++] = (char) (0x80 | ((codepoint >> 6) & 0x3f));
    data->buff[data->pos++] = (char) (0x80 | (codepoint & 0x3f));
    ...
    data->buff[data->pos++] = (char) (0xf0 | (codepoint >> 18));
    data->buff[data->pos++] = (char) (0x80 | ((codepoint >> 12) & 0x3f));
    data->buff[data->pos++] = (char) (0x80 | ((codepoint >> 6) & 0x3f));
    data->buff[data->pos++] = (char) (0x80 | (codepoint & 0x3f));
  } else {
    std::cerr << "ERROR";
...
void emit_image(struct color_data *data, const cimg_library::CImg<unsigned char> & image, int flags) {
    for (int y = 0; y <= image.height() - 8; y += 8) {
      for (int x = 0; x <= image.width() - 4; x += 4) {
        ...
        emit_color(data, flags | FLAG_BG, charData.bgColor[0], charData.bgColor[1], charData.bgColor[2]);
        emit_color(data, flags | FLAG_FG, charData.fgColor[0], charData.fgColor[1], charData.fgColor[2]);
        emitCodepoint(data, charData.codePoint);
      }
      // std::cout << "\x1b[0m" << std::endl;
      realloc_color_data(data);
      data->pos += sprintf(data->buff, "\x1b[0m\n");
    }
}

enum Mode {AUTO, THUMBNAILS, FULL_SIZE};

extern "C" void* encode2data(const char *file, int *size);

void* encode2data(const char *file, int *size) {
  struct winsize w;
...
extern "C" void* encode2datafromdata(const unsigned char *data, int *size);

void* encode2datafromdata(const unsigned char *mem, int *size) {
  struct winsize w;
  ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);
  ...
  if (mode == FULL_SIZE || (mode == AUTO && file_names.size() == 1)) {
      try {
        cimg_library::CImg<unsigned char> image(mem, (unsigned int)*size);
